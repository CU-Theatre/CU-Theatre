...
export const WindowSwiper: React.FC = () => {
  ...
  return (
    <div className={styles["window-slider"]}>
      ...
      >
        {slides.map(slide => (
          <SwiperSlide className={styles["window-slider__slide"]} key={slide.slideTitle}>
            <h4 className={styles["window-slider__title"]}>{slide.slideTitle}</h4>
            <div className={styles['window-slider__content']}>
              <img className={styles["window-slider__image"]} src={loadedImages[slide.slideTitle]}  alt={slide.slideTitle} />
     ...
     <div className={styles["window-slider__buttons"]}>
      <button type="button" className="slider-button slider-button--prev"></button>
      <button type="button" className="slider-button slider-button--next"></button>
     </div>
    </div>
  );
};